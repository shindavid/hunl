...
BettingDecision RandomPlayer::makeDecision(const HandState& state) {
  ...
  if (call_amount==0) {
    ...
    switch (choice) {
      case random_params_t::CHECK: return _createBettingDecision(state, 0);
    ...
    switch (choice) {
      case random_params_t::FOLD: return _createBettingDecision(state, 0);
      case random_params_t::CALL: return _createBettingDecision(state, call_amount);
